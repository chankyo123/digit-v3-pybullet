...
class Card( object ):
    ...
    def __str__( self ):
        ...
        if self.__face_up:
            if not self.playable and not self.used:
                return "{}{}".format( (self.rank_list)[self.__rank], \
                              (self.suit_list)[self.__suit] )
            elif self.used:
                out = Fore.BLUE + "{}{}".format((self.rank_list)[self.__rank], \
                                               (self.suit_list)[self.__suit]) + Style.RESET_ALL
                return out
            else:
                out = Fore.RED + "{}{}".format ((self.rank_list)[self.__rank], \
                                                (self.suit_list)[self.__suit]) + Style.RESET_ALL
                return out
        else:
            if self.playable:
                out = Fore.RED + "{}{}".format("X", "X") + Style.RESET_ALL
                return out
            else:
                return "{}{}".format( "X", "X")
